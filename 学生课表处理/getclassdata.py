...
file_rootdir = 'file/'
lists = os.listdir(file_rootdir)
for eachstation in lists:
    station_dir = os.path.join(file_rootdir,eachstation)
    html_dir = os.path.join(station_dir,"class_messege")
    list = os.listdir(html_dir)
    for eachone in list:
        path = os.path.join(html_dir, eachone)
        class_file = open(path, "r", encoding="utf-8")
        class_file = class_file.read()
        json_path = os.path.join(station_dir,"json",eachone[0]+".json")
